...
export function hasViewAccess(role: string, view: View): boolean {
  if (view === "admin" || view === "projects" || view === "categories" || view === "users" || view === "holidays" || view === "leave-policies" || view === "work-policies" || view === "branding" || view === "retention-policy" || view === "audit-logs" || view === "password-policy" || view === "billing") return role === "admin";
...
function AppRoutes() {
  ...
  const nav = useMemo(
    () => ["dashboard", "timesheets", "leave", "reports", ...(isManager ? ["approvals", "team", "capacity"] : []), ...(isAdmin ? ["admin", "projects", "categories", "users", "holidays", "leave-policies", "work-policies", "branding", "retention-policy", "audit-logs", "password-policy", "billing"] : [])] as View[],
    [isAdmin, isManager]
  );
    ...
    <SignalRProvider userId={session.userId} managerId={session.role === "manager" || session.role === "admin" ? session.userId : null}>
      ...
      >
        <Routes>
          ...
          {isAdmin   && <Route path="/billing"          element={<ErrorBoundary><Billing /></ErrorBoundary>} />}
          <Route path="*" element={<Navigate to="/dashboard" replace />} />
        </Routes>
      </AppShell>
    </SignalRProvider>
